...
type SortedSet{K, Ord <: Ordering}
    ...
    SortedSet(o::Ord=Forward) = new(BalancedTree23{K,Void,Ord}(o))

    function SortedSet(o::Ord, iter)
...
if VERSION < v"0.5"
    ...
    SortedSet(o1::Ordering,o2::Ordering) = throw(ArgumentError("SortedSet with two parameters must be called with an Ordering and an interable"))
end
...
if VERSION < v"0.5"
    # To address ambiguity warnings on Julia v0.4
    @compat (::Type{SortedSet{K}}){K}(o1::Ordering,o2::Ordering) = throw(ArgumentError("SortedSet with two parameters must be called with an Ordering and an interable"))
end
